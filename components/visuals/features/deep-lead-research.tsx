  ...
  return (
    <FeatureVisualFrame>
      <svg viewBox="0 0 420 250" className="h-full w-full" role="img" aria-label="Research signal intensity over time">
        ...
        <motion.line x1="320" x2="320" y1="30" y2="224" stroke="var(--color-oraami-accent-1)" strokeWidth="1.5" strokeDasharray="4 4" animate={loop ? { opacity: [0, 0, 1, 1, 0] } : { opacity: 1 }} transition={loop ? { duration: VISUAL_DURATION, times: [0, 0.54, 0.64, 0.92, 1], repeat: Infinity, repeatDelay: 1 } : { duration: 0 }} />
        <motion.circle cx="320" cy="96" r="5" fill="var(--color-oraami-accent-1)" animate={loop ? { r: [3, 3, 7, 5, 3], opacity: [0, 0, 1, 1, 0] } : { r: 5, opacity: 1 }} transition={loop ? { duration: VISUAL_DURATION, times: [0, 0.54, 0.66, 0.92, 1], repeat: Infinity, repeatDelay: 1 } : { duration: 0 }} />
        <motion.text x="244" y="118" fontSize="12" fontWeight="500" fill="var(--color-oraami-accent-25)" animate={loop ? { opacity: [0, 0.25, 1, 1, 0] } : { opacity: 1 }} transition={loop ? { duration: VISUAL_DURATION, times: [0, 0.34, 0.5, 0.92, 1], repeat: Infinity, repeatDelay: 1 } : { duration: 0 }}>Growth</motion.text>
        <motion.text x="326" y="84" fontSize="12" fontWeight="500" fill="var(--color-oraami-accent-1)" animate={loop ? { opacity: [0, 0, 1, 1, 0], y: [4, 4, 0, 0, -2] } : { opacity: 1, y: 0 }} transition={loop ? { duration: VISUAL_DURATION, times: [0, 0.56, 0.68, 0.92, 1], repeat: Infinity, repeatDelay: 1 } : { duration: 0 }}>Buying Signal</motion.text>
      </svg>
