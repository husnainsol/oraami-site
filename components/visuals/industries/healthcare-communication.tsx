...
import { INDUSTRY_COLORS, INDUSTRY_DURATION, INDUSTRY_SVG_CLASS, IndustryVisualFrame, MiniArrow, SceneCard, SceneConnector, SceneNode, SceneOverline } from "./shared"

export function HealthcareCommunicationVisual({ reduceMotion }: IndustryVisualProps) {
  const { orange, white, border, panel } = INDUSTRY_COLORS

  return (
    <IndustryVisualFrame reduceMotion={reduceMotion}>
      <svg viewBox="0 0 420 180" preserveAspectRatio="xMidYMid meet" className={INDUSTRY_SVG_CLASS} role="img" aria-label="Healthcare communication route">
        <SceneOverline>CARE COMMUNICATION ROUTE</SceneOverline>
        ...
        <path d="M126 91 C150 91 160 55 181 55 M126 91 H189 M126 91 C150 91 160 127 181 127" fill="none" stroke="rgba(255,255,255,0.16)" strokeWidth="1.4" strokeLinecap="round" />
        ...
        <motion.circle cx="219" cy="91" r="4" fill={orange} animate={reduceMotion ? { x: 0, opacity: 0 } : { x: [0, 0, 79, 79], opacity: [0, 1, 1, 0] }} transition={reduceMotion ? { duration: 0 } : { duration: INDUSTRY_DURATION, repeat: Infinity, times: [0, 0.38, 0.68, 1], ease: "easeInOut" }} />
